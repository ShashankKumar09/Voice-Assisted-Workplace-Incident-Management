...
def save_incident_record(
    ...
    if (
        ...
        existing_df = existing_df[
            existing_df["ID"].astype(str).str.strip()
            != incident_id
        ]
...
def render_prediction_results(
    ...
    st.success(
        ...
        "Review the predicted OSHA categories, confidence scores and "
        "decision recommendation before downloading the final report."
    )
    ...
    st.caption(
        "Download the classified incident record "
...
def render() -> None:
    ...
    with st.form(
        ...
        st.markdown(
            "### Incident Outcomes and References"
        )
    ...
    if (
        st.session_state.manual_result
        is not None
        and st.session_state.manual_report_package
        is not None
    ):
