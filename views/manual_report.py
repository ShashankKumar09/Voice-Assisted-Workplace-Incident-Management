...
def load_predictor():
    """Load the packaged multi-task DeBERTa predictor when needed."""
    from core.predictor import IncidentPredictor

    return IncidentPredictor(application_root=APP_ROOT, device="auto")
...
def save_incident_record(report_package: Dict[str, Any]) -> None:
    ...
    incident_id = str(record_df.iloc[0].get("ID", "")).strip()
    if not existing_df.empty and "ID" in existing_df.columns and incident_id:
...
def _decision_display(decision_tier: str) -> tuple[str, str]:
...
def _render_prediction_card(
    ...
    confidence_clamped = max(0.0, min(float(confidence), 100.0))
    display_code = str(code).strip() or "Not available"
    ...
    st.html(
        ...
        ">
            <div style="
                color:#6A7F8F;font-size:0.72rem;font-weight:800;
                letter-spacing:0.08em;text-transform:uppercase;
            ">{title}</div>

            <div style="
                margin-top:0.65rem;color:#2B7897;font-size:1.05rem;
                font-weight:850;
            ">Code: {display_code}</div>

            <div style="
                min-height:78px;margin-top:0.45rem;color:#17324D;
                font-size:1.08rem;line-height:1.35;font-weight:800;
            ">{label}</div>

            <div style="
                height:9px;margin-top:1rem;border-radius:999px;
                overflow:hidden;background:#E7EEF2;
            ">
                <div style="
                    width:{confidence_clamped:.2f}%;height:100%;
                    border-radius:999px;
                    background:linear-gradient(90deg,#1F6A86,#2B8BA5);
                "></div>
            </div>
            ...
            <div style="
                ...
                margin-top:0.65rem;
            ">
                <span style="color:#2B7897;font-size:0.82rem;font-weight:800;">
                    {confidence_clamped:.2f}%
                </span>
                <span style="color:#7C8E9A;font-size:0.73rem;">Confidence</span>
            </div>
        </div>
        """
    )
...
def _render_summary_card(
    ...
    st.html(
        ...
        ">
            ...
            ">{label}</div>
            <div style="
                margin-top:0.65rem;color:{value_color};font-size:1.32rem;
                line-height:1.3;font-weight:850;
            ">{value}</div>
            <div style="
                margin-top:0.45rem;color:#7C8E9A;font-size:0.75rem;
                line-height:1.45;
            ">{caption}</div>
        </div>
        """
...
def render_prediction_results(
    ...
    """Render classification results with OSHA/OIICS codes and titles."""
    enrich_prediction_result(prediction)

    st.success(
        "✅ Incident classified successfully. Review the predicted OSHA/OIICS "
        "codes, titles, confidence scores and decision recommendation before "
    ...
    for column, (task_name, display_name) in zip(
        ...
        with column:
            _render_prediction_card(
                title=display_name,
                code=str(task_result.get("code", "")),
                label=str(
                    task_result.get("title", task_result.get("label", ""))
                ),
    ...
    with summary_columns[0]:
        _render_summary_card(
            ...
            caption="Geometric mean across Nature, Part of Body, Event and Source.",
        )
...
def render() -> None:
    ...
    render_user_guide(
        ...
        steps=[
            ("Open a new incident form", "Begin a structured single-incident report."),
            (
            ...
            (
                ...
                "Review codes, titles, confidence, decision tier and report files.",
            ),
        ],
        note=(
    ...
    with st.form("manual_incident_form", clear_on_submit=False):
        ...
        incident_id = c1.text_input("Incident ID *", placeholder="INC-1001")
        upa = c2.text_input("UPA", placeholder="UPA-2501")
        event_date = c3.date_input("Event Date", value=date.today())

        st.markdown("### Employer and Location")
        c1, c2 = st.columns(2)
        employer = c1.text_input("Employer", placeholder="ABC Manufacturing")
        primary_naics = c2.text_input("Primary NAICS", placeholder="332710")
        address1 = st.text_input("Address Line 1", placeholder="100 Industrial Road")
        address2 = st.text_input("Address Line 2", placeholder="Building B")
        c1, c2, c3 = st.columns(3)
        city = c1.text_input("City", placeholder="Bengaluru")
        state = c2.text_input("State", placeholder="Karnataka")
        zip_code = c3.text_input("ZIP / Postal Code", placeholder="560001")
        c1, c2 = st.columns(2)
        latitude = c1.text_input("Latitude", placeholder="12.9716")
        longitude = c2.text_input("Longitude", placeholder="77.5946")

        st.markdown("### Incident Outcomes and References")
        c1, c2, c3 = st.columns(3)
        hospitalized = c1.selectbox("Hospitalized", ["", "Yes", "No"])
        amputation = c2.selectbox("Amputation", ["", "Yes", "No"])
        loss_of_eye = c3.selectbox("Loss of Eye", ["", "Yes", "No"])
        c1, c2 = st.columns(2)
        inspection = c1.text_input("Inspection", placeholder="INS-45021")
        ...
        final_narrative = st.text_area(
            ...
            height=170,
            placeholder=(
                "Example: An employee slipped on a wet floor, fell on the same "
                "level, and fractured the left ankle."
            ),
            help=(
                "Include the activity, event, injury, body part and source where known."
            ),
        )
    ...
    if submitted:
        ...
        narrative_word_count = len(final_narrative.strip().split())
        if len(final_narrative.strip()) < 15 or narrative_word_count < 4:
            errors.append("Final Narrative must contain a clear incident description.")
            ...
            if value.strip():
                try:
                    ...
                    if not minimum <= numeric_value <= maximum:
                        errors.append(
                            f"{field_label} must be between {minimum:g} and {maximum:g}."
                        )
        ...
        if errors:
            ...
            try:
                with st.spinner("Loading the model and classifying the incident..."):
                    ...
                    enrich_prediction_result(prediction)
                    report_package = generate_report(incident_details, prediction)
                    save_incident_record(report_package)
    ...
    if (
        ...
        st.markdown("")
        if st.button("Start New Manual Report", use_container_width=True):
            st.session_state.manual_result = None
            st.session_state.manual_report_package = None
            st.rerun()
